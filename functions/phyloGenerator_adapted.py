#!/usr/bin/env python
## encoding: utf-8
## Adapted phyloGenerator (http://willpearse.github.com/phyloGenerator)
## D. J. Bennnett
## ADAPTATIONS:
# 1. Beginning lines of sequenceDownload()
...
def findLineage(spName):
	"""Take a species name and return its lineage. Takes either name or taxon ID."""
	try:
		if isinstance(spName, str):
			...
			handleID = Entrez.efetch(db="taxonomy", id=spName, retmode="xml")
			resultsSpID = Entrez.read(handleID)
			lineageEx = resultsSpID[0]["lineageEx"]
			lineage = [each['TaxId'] for each in lineageEx]
		return lineage
	except:
		return ()
...
def findRelativeSequence(spName, geneName=None, cladeDepth=0, thorough=False, rettype='gb', noSeqs=1, seqChoice='random', download=True, retStart=0, retMax=20, targetLength=None, trimSeq=False, DNAtype='Standard', gapType='-', includeGenome=True, includePartial=True):
	if isinstance(spName, str):
		...
		cladeNames = findLineage(spName)
		genusName = cladeNames[0]
	namesTried = [genusName]
	genusDownload, _ = sequenceDownload(spName, geneName, thorough, rettype, noSeqs, seqChoice, download, retStart, retMax, targetLength, trimSeq, DNAtype, gapType, includeGenome, includePartial) # shouldn't this have genusName?
	if genusDownload:
...
def sequenceDownload(spName, geneName, thorough=False, rettype='gb', noSeqs=1, seqChoice='random', download=True, retStart=0, retMax=20, targetLength=None, trimSeq=False, DNAtype='Standard', gapType='-', includeGenome=True, includePartial=True):
	def dwnSeq(includeGenome, includePartial, gene):
		if isinstance(spName, int):
			searchTerm = "txid" + str(spName) + "[Primary Organism]"
		else:
			searchTerm = spName + "[Primary Organism]" # Primary to avoid viruses for example
