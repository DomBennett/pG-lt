...
fewer matches than target nseqs"""
		...
		while len(seqids) < self.nseqs:
			if self.thoroughness > self.max_thoroughness:
				...
				break
			search_term = self._buildSearchTerm(taxids,\
				self.thoroughness)
			...
			if int(seqcount) >= 1:
				...
				seqids = [e for e in seqids if not e in\
				self.deja_vues]
			self.thoroughness += 1
		...
		while self.thoroughness < self.max_thoroughness:
			...
			if len(seqids) >= self.nseqs*10:
				...
				len(seqids):
					...
					downloaded.extend(self._download(seqids[lower:\
						upper]))
					if not downloaded:
						break
...
matches in GenBank"""
	def nextBest (searchlist, types):
		...
		# else find all types for each gene
			...
			search_types = [genedict[e[0]]['type'] for e in \
			searchlist]
